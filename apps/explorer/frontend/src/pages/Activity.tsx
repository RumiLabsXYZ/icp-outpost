...
export function Activity() {
  ...
  function updateFilters(patch: Partial<ActivityFilters>) {
    const next: ActivityFilters = { ...filters, ...patch, cursor: null };
    ...
    <div>
        ...
        <div className="bg-vellum-raised border border-quartz rounded-md p-4 mb-6">
          ...
          <div className="flex flex-wrap gap-1.5">
            {EVENT_KINDS.map((k) => (
              ...
              >
                <span className="mr-1">{k.glyph}</span>{k.label}
              </button>
            ))}
      ...
      {error && (
        <div className="bg-vellum-raised border border-quartz rounded-md p-6 text-sm text-ink-muted">
          <p className="font-medium text-ink-primary mb-1">Activity feed unavailable.</p>
          <p>Check that your source canister exposes <code className="font-mono">get_events_filtered</code> and that AppEvents.mo matches its response shape.</p>
        </div>
      )}

      {data && data.events.length === 0 && !error && !isLoading && (
        <div className="bg-vellum-raised border border-quartz rounded-md p-6 text-sm text-ink-muted">
          <p className="font-medium text-ink-primary mb-1">No events found.</p>
          <p>Wire <code className="font-mono">get_events_filtered</code> in your source canister and update AppEvents.mo to match.</p>
        </div>
      )}
